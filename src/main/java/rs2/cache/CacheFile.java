...
public final class CacheFile {
     ...
     */
    private final byte[] cacheFileBuffer;
    ...
    public final byte[] getContainerUnpackedData(int containerId, int[] xteaKeys) {
        ...
        if (xteaKeys != null && (xteaKeys[0] != 0 || xteaKeys[1] != 0 || xteaKeys[2] != 0 || xteaKeys[3] != 0)) {
            packedData = XTEACypher.decrypt(xteaKeys, ByteBuffer.wrap(packedData), 5, packedData.length).array();
        }
        return ContainerSegmentData.unpackCacheContainer(packedData);
    }
    ...
    public final byte[] getContainerData(int containerId) {
        synchronized (dataFile) {
            try {
                if (indexFile.length() < (6L * containerId + 6)) {
                    return null;
                }
